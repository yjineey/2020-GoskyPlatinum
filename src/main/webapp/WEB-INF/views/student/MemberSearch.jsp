			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div
						...
						<h1 class="h3 mb-0 text-gray-800">회원리스트</h1>
						<a href="#"
					...
					<div class="container-fluid">
						...
						<!-- Page Heading -->
						<p class="mb-4">
							관리자에게 보여지는 회원 리스트화면 입니다 
						</p>

						<!-- DataTales Example -->
						<div class="card shadow mb-4">
							<div class="card-header py-3">
								<h6 class="m-0 font-weight-bold text-primary">회원검색</h6>
							</div>
									...
									<!-- 검색 -->
									    ...
									    <div class="form-group col-sm-2">
									        <select class="form-control" name="searchType" id="searchType">
									            ...
									            <option value="t" <c:out value="${searchPaging.searchType eq 't' ? 'selected' : ''}"/>>이름</option>
									            <option value="c" <c:out value="${searchPaging.searchType eq 'c' ? 'selected' : ''}"/>>번호</option>
									        </select>
									    </div>
									...
									<table class="table table-bordered" id="dataTable">
											...
											<c:choose>
												<c:when test="${fn:length(MemberList) > 0}">
													<c:forEach var="list" items="${MemberList }">
														<tr>
														<td>${list.mno}</td>
															<td><a href="${ContextPath}/MemberMypageUpdate?mid=${list.mid}">${list.mname}</a></td>
															<td>${list.memail}</td>
															<td>${list.number}</td>
										...
										<!--<c:if test="${msort eq 'm03'}">
											<td colspan="6" align="right"><input
												class="btn btn-success" type="button" value="원생 리스트 엑셀 저장"
												id="write" /></td></c:if>-->
										</tr>
									</table>
									<nav aria-label="Page navigation">
											<ul class="pagination" text-align:center>
												...
												<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
													<li class="page-item"><a class="page-link" href="${ContextPath}/MemberSearch${pageMaker.makeSearch(idx)}">${idx}</a>
													</li>
												</c:forEach>
